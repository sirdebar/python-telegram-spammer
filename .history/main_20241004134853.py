...
async def process_photo_message(message: Message, state: FSMContext):
    ...
    photo = message.photo[-1]  # Фото с максимальным разрешением
...
async def start_mailing(account_id, chats, messages, delay, user_id):
    """Запуск рассылки сообщений с поддержкой фото и текста."""
    ...
    # Проверяем наличие данных для рассылки
    photo = user_data.get('photo')  # Фото, если есть
    caption = user_data.get('caption', '')  # Подпись к фото, если есть
    messages = user_data.get('messages', None)  # Сообщение, если есть текст

    # Если нет ни текста, ни фото - ошибка
    if not messages and not photo:
        await client.send_message(user_id, "<b>Ошибка:</b> Нет данных для рассылки. Убедитесь, что вы отправили текст или фото.")
        return
