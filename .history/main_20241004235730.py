...
async def start_bot(message: Message, state: FSMContext):
    ...
    # Проверяем, есть ли пользователь в базе данных и подписан ли он
    async with aiosqlite.connect('bot_database.db') as db:
        async with db.execute('SELECT subscribed FROM users WHERE user_id = ?', (user_id,)) as cursor:
            user = await cursor.fetchone()

    # Если пользователь не найден в базе или не подписан, проверяем его статус
    if not user or not user[0]:
        await message.answer(
            "<b>👋 Приветствуем вас в нашем боте!</b>\n"
            "Чтобы пользоваться ботом, вам необходимо подписаться на наш канал.",
            reply_markup=get_subscription_keyboard()
        )
    else:
        # Пользователь подписан, сразу предоставляем доступ
...
async def check_subscription(callback_query: CallbackQuery):
    ...
    try:
        ...
        if member.status in ['member', 'administrator', 'creator']:
            # Пользователь подписан, удаляем сообщение о подписке и даем доступ
            await callback_query.message.delete()

            # Обновляем статус подписки в базе данных
            async with aiosqlite.connect('bot_database.db') as db:
                await db.execute('INSERT OR REPLACE INTO users (user_id, subscribed) VALUES (?, ?)', (user_id, True))
                await db.commit()

            # Предоставляем доступ
            await give_access(callback_query.message)
        else:
            # Если статус не 'member', показываем сообщение снова
            await callback_query.message.answer(
                "<b>Вы должны подписаться на канал, чтобы продолжить пользоваться ботом.</b>",
                reply_markup=get_subscription_keyboard()
            )
    except TelegramBadRequest:
        # Если произошла ошибка при проверке статуса, показываем сообщение о необходимости подписки
        await callback_query.message.answer(
            "<b>Перед использованием бота подпишитесь на канал.</b>",
            reply_markup=get_subscription_keyboard()
        )
