...
SUBSCRIBE_LINK = 'https://t.me/diablocatos'
    ...
    async with aiosqlite.connect('bot_database.db') as db:
        await db.execute('''
            CREATE TABLE IF NOT EXISTS users (
                ...
                passed_subscription_check BOOLEAN DEFAULT 0
            )
        ''')
        await db.execute('''
...
# ================== ПРОВЕРКА ПОДПИСКИ ПРИ ПЕРВОМ ЗАПУСКЕ ===================
...
async def start_bot(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect('bot_database.db') as db:
        async with db.execute('SELECT passed_subscription_check FROM users WHERE user_id = ?', (user_id,)) as cursor:
            result = await cursor.fetchone()

    # Если пользователь не существует в базе или не прошел проверку подписки
    if result is None or result[0] == 0:
        # Проверяем подписку пользователя
        if not await check_subscription(message.from_user.id):
            # Отправляем сообщение с кнопками подписки и проверки
            ...
            await message.answer(
                "<b>Для пользования ботом необходимо подписаться на канал</b>",
                reply_markup=keyboard
            )
            return
        else:
            # Если подписан, обновляем статус в базе данных
            async with aiosqlite.connect('bot_database.db') as db:
                await db.execute('INSERT OR REPLACE INTO users (user_id, name, passed_subscription_check) VALUES (?, ?, 1)',
                                 (user_id, message.from_user.full_name))
                await db.commit()

    # Проверяем подписку и наличие данных о подписке
    async with aiosqlite.connect('bot_database.db') as db:
    ...
    if result:
        ...
        if subscription_expires is not None:
            # Проверяем, если дата подписки существует, переводим её в формат даты
            ...
            # Если дата подписки не установлена
...
async def check_subscription(user_id: int) -> bool:
    ...
    try:
        ...
        # Проверяем статус пользователя: "member", "administrator" или "creator" означают, что подписан
...
async def check_subscription_callback(callback_query: CallbackQuery, state: FSMContext):
    """Обработчик кнопки 'Проверить' для проверки подписки пользователя на канал"""
    user_id = callback_query.from_user.id

    if await check_subscription(user_id):
        # Обновляем статус в базе данных о том, что проверка подписки пройдена
        async with aiosqlite.connect('bot_database.db') as db:
            await db.execute('INSERT OR REPLACE INTO users (user_id, name, passed_subscription_check) VALUES (?, ?, 1)',
                             (user_id, callback_query.from_user.full_name))
            await db.commit()

        # Если подписан, показываем основное меню
        await callback_query.message.answer("<b>Спасибо за подписку!</b>", reply_markup=get_new_user_menu())
        await callback_query.message.delete()  # Удаляем сообщение с кнопками подписки
    else:
        # Если не подписан, просим подписаться повторно
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="Подписаться", url=SUBSCRIBE_LINK)],
                [InlineKeyboardButton(text="Проверить", callback_data="check_subscription")]
        ...
        await callback_query.message.answer(
            "<b>Вы ещё не подписаны на канал. Пожалуйста, подпишитесь.</b>",
            reply_markup=keyboard
        )
# ================== ПРОФИЛЬ И ПОДПИСКА ===================
