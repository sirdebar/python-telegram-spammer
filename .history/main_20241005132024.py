...
async def start_mailing(account_id, chats, messages, photo, delay, user_id):
    ...
    # Отправляем первое сообщение без задержки
    first_chat = chats[0]  # Первая группа/чат для рассылки
    try:
        if photo:
            await client.send_file(first_chat, photo, caption=messages if messages else "")
        else:
            await client.send_message(first_chat, messages)
        sent_count += 1
    except Exception as e:
        logging.error(f"Ошибка при отправке в чат {first_chat}: {e}")

    # Удаляем первый чат из списка, чтобы не отправлять туда повторно
    remaining_chats = chats[1:]

    # Применяем задержку и отправляем в оставшиеся чаты
    while active_mailings.get(user_id) and remaining_chats:
        for chat_id in remaining_chats:
