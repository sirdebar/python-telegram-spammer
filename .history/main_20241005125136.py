...
async def change_channel_prompt(message: Message, state: FSMContext):
    ...
    if user_id == ADMIN_ID:
        await message.answer("<b>Введите @username нового канала для обязательной подписки:</b>")
...
async def set_new_channel_name(message: Message, state: FSMContext):
    ...
    if not new_channel_name.startswith('@'):
        await message.answer("<b>⚠️ Ошибка: Публичное имя канала должно начинаться с @.</b>")
        return

    CHANNEL_NAME = new_channel_name  # Обновляем глобальную переменную
