...
async def start_bot(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect('bot_database.db') as db:
        async with db.execute('SELECT accepted_terms, is_subscribed, subscription_expires FROM users WHERE user_id = ?', (user_id,)) as cursor:
            result = await cursor.fetchone()
            logging.info(f"User {user_id} data fetched: {result}")  # Логирование данных пользователя
    ...
    if result:
        ...
        if not accepted_terms:
            logging.info(f"User {user_id} has not accepted the terms. Sending agreement message.")  # Лог
            sent_message = await message.answer(
                ...
                reply_markup=get_agreement_keyboard()  # Добавляем инлайн-кнопки под сообщением
            )
            await state.update_data(message_id=sent_message.message_id)
            return

        # Если пользователь уже принял условия, продолжаем с подпиской
        ...
        if is_subscribed and subscription_expires > datetime.now():
            logging.info(f"User {user_id} subscription is active.")  # Лог
            ...
            try:
                logging.info(f"Checking subscription for user {user_id}.")  # Лог
        ...
        logging.info(f"User {user_id} is not registered. Requesting subscription.")  # Лог
...
async def accept_agreement(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Переводим пользователя к подписке
