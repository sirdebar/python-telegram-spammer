...
async def delete_account(callback_query: CallbackQuery):
    ...
    # Получаем номер телефона и сессию из базы данных
    cursor.execute('SELECT phone_number, session FROM accounts WHERE account_id = ?', (account_id,))
    account = cursor.fetchone()
    if not account:
        await callback_query.message.answer("<b>Ошибка:</b> Аккаунт не найден.")
        return
    
    phone_number, session_string = account
    session_path = f'sessions/{phone_number}.session'

    # Инициализируем клиента Telethon для корректного завершения сессии
    client = TelegramClient(session_path, TELEGRAM_API_ID, TELEGRAM_API_HASH)
    await client.connect()

    if await client.is_user_authorized():
        await client.log_out()  # Завершаем активную сессию
