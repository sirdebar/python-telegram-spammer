...
async def process_messages(message: Message, state: FSMContext):
    ...
    if message.photo:
        # Если сообщение содержит фото, сохраняем фото и подпись (если есть)
        photo = message.photo[-1]  # Берем самое большое разрешение фото
        caption = message.caption if message.caption else ""  # Если есть подпись
        await state.update_data(messages={"type": "photo", "photo": photo, "caption": caption})
        await message.answer("<b>Фото принято для рассылки.</b>")
    elif message.text:
        # Если сообщение текстовое
        await state.update_data(messages={"type": "text", "text": message.text})
        await message.answer("<b>Текстовое сообщение принято для рассылки.</b>")
    else:
        await message.answer("<b>⚠️ Сообщение должно содержать либо текст, либо фото.</b>")
        return
...
async def send_messages(account_id, chats, messages_data, delay, user_id):
    cursor.execute('SELECT phone_number, session FROM accounts WHERE account_id = ?', (account_id,))
    account = cursor.fetchone()
    ...
    for chat in chats:
        try:
            if message_type == "text":
                ...
                text_message = messages_data["text"]  # Убери лишние манипуляции с кодировкой
                await client.send_message(chat, text_message)
            elif message_type == "photo":
                # Отправляем фото с подписью
                caption_message = messages_data["caption"]  # Убери кодировку для подписи
                await client.send_file(chat, messages_data["photo"], caption=caption_message)
            
            sent_count += 1
            with db_lock:
                cursor.execute('INSERT INTO mailings (user_id, chats, messages, status, sent_messages, start_time) VALUES (?, ?, ?, ?, ?, ?)',
                               (user_id, ','.join(str(c) for c in chats), str(messages_data), 'active', sent_count, datetime.now().isoformat()))
                conn.commit()
...
async def start_mailing(account_id, chats, messages_data, delay, user_id):
    ...
    while active_mailings.get(user_id):
        for chat_id in chats:
            if not active_mailings.get(user_id):
                break
            try:
                # Отправляем сообщение в выбранный чат в зависимости от типа (текст или фото)
                if messages_data["type"] == "text":
                    await client.send_message(chat_id, messages_data["text"])
                elif messages_data["type"] == "photo":
                    await client.send_file(chat_id, messages_data["photo"], caption=messages_data["caption"])
