...
def get_agreement_keyboard():
    # Это инлайн-кнопки под сообщением
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
...
async def start_bot(message: Message, state: FSMContext):
    ...
    # Проверка статуса пользователя (принял ли соглашение и подписан ли)
    async with aiosqlite.connect('bot_database.db') as db:
    ...
    if result:
        ...
        subscription_expires = datetime.strptime(subscription_expires_str, "%Y-%m-%d %H:%M:%S") if subscription_expires_str else None

        # Если пользователь принял соглашение, подписан и подписка активна
        if accepted_terms and is_subscribed and subscription_expires and subscription_expires > datetime.now():
            sent_message = await message.answer("<b>🎉 Ваш доступ активен!</b>", reply_markup=get_user_menu())
            await state.update_data(message_id=sent_message.message_id)
            return
    else:
    ...
    # Если пользователь не принял соглашение, предлагаем ему его принять
    if not result or not accepted_terms:
    ...
    # Если пользователь принял соглашение, но не подписан — проверка подписки
...
async def accept_agreement(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Удаляем сообщение с предложением о соглашении
    previous_message_data = await state.get_data()
    if "message_id" in previous_message_data:
        try:
    ...
    # Переходим к следующему этапу — проверка подписки
    await check_subscription(callback_query, state)


async def check_subscription(event: Union[Message, CallbackQuery], state: FSMContext):
    ...
    if "message_id" in previous_message_data:
        try:
            await bot.delete_message(chat_id=event.message.chat.id, message_id=previous_message_data['message_id'])
        except Exception as e:
            logging.error(f"Ошибка при удалении сообщения: {e}")

    # Проверка подписки через Telegram API
    try:
        member_status = await bot.get_chat_member(chat_id=CHANNEL_NAME, user_id=user_id)  # Используем CHANNEL_NAME
    except Exception as e:
        logging.error(f"Ошибка при проверке подписки через Telegram API: {e}")
        await event.answer("<b>⚠️ Ошибка при проверке подписки. Пожалуйста, попробуйте позже.</b>")
        return

    if member_status.status in ["member", "administrator", "creator"]:
        # Пользователь подписан на канал, проверяем платную подписку
        async with aiosqlite.connect('bot_database.db') as db:
        ...
        if result:
            ...
            subscription_expires = (
                datetime.strptime(subscription_expires_str, "%Y-%m-%d %H:%M:%S")
                if subscription_expires_str else None
            )

            # Если платная подписка активна
            if is_subscribed and subscription_expires and subscription_expires > datetime.now():
                sent_message = await event.message.answer("<b>🎉 Спасибо за подписку! Ваш доступ активен.</b>", reply_markup=get_user_menu())
            else:
                # Платная подписка истекла или отсутствует
                sent_message = await event.message.answer("<b>🎉 Спасибо за подписку! Теперь купите доступ для использования бота.</b>", reply_markup=get_new_user_menu())
        else:
        ...
        await event.message.answer("<b>Для использования бота, подпишитесь на канал:</b>", reply_markup=get_subscription_keyboard())
        return
