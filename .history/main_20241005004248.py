...
class DeletePreviousMessageMiddleware(BaseMiddleware):
    def __init__(self):
        super().__init__()
        # Словарь для хранения предыдущих сообщений (ключ — ID пользователя)
    ...
    async def __call__(self, handler, event, data):
        if isinstance(event, Message):
            ...
            bot: Bot = data['bot']  # Доступ к объекту бота через data

            # Удаляем предыдущее сообщение, если оно существует
            if user_id in self.previous_messages:
                previous_message_id = self.previous_messages[user_id]
                try:
                    await bot.delete_message(chat_id=chat_id, message_id=previous_message_id)
                except Exception as e:
                    print(f"Ошибка при удалении сообщения: {e}")

            # Обрабатываем текущее сообщение и сохраняем его ID
            result = await handler(event, data)
            
            # Сохраняем ID отправленного сообщения
            self.previous_messages[user_id] = event.message_id
            return result
        else:
