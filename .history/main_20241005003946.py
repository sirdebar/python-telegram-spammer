...
class DeletePreviousMessageMiddleware:
    def __init__(self):
    ...
    async def on_pre_process_message(self, message: Message, data: dict):
        ...
        # Если есть предыдущее сообщение для этого пользователя, удаляем его
        if user_id in self.previous_messages:
            try:
                await message.bot.delete_message(chat_id, self.previous_messages[user_id])
            except Exception as e:
                logging.error(f"Ошибка при удалении сообщения: {e}")

        # Ожидаем отправки сообщения и сохраняем его ID
        data["delete_previous"] = user_id  # Для передачи данных в post process

    async def on_post_process_message(self, message: Message, result, data: dict):
        user_id = data.get("delete_previous")
        if user_id:
            # Сохраняем ID нового сообщения
            self.previous_messages[user_id] = message.message_id
...
async def main():
    ...
    dp.message.middleware(DeletePreviousMessageMiddleware())  # Регистрируем middleware
