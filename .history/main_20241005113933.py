...
async def confirm_send_announcement(callback_query: CallbackQuery, state: FSMContext):
    ...
    if data['content_type'] == "text":
        ...
        for user in users:
            try:
                await bot.send_message(user_id=user[0], text=content)
            except Exception as e:
                logging.error(f"Ошибка при отправке сообщения пользователю {user[0]}: {e}")
        # Редактируем текст сообщения для подтверждения отправки
        await callback_query.message.edit_text("<b>✅ Сообщение успешно разослано всем пользователям!</b>")
    elif data['content_type'] == "photo":
        photo_id = data['photo']
        caption = data.get('caption', "")
        # Рассылка фото всем пользователям
        for user in users:
            try:
                await bot.send_photo(user_id=user[0], photo=photo_id, caption=caption)
            except Exception as e:
                logging.error(f"Ошибка при отправке фото пользователю {user[0]}: {e}")
        # Редактируем подпись к сообщению для подтверждения отправки
        await callback_query.message.edit_caption("<b>✅ Сообщение успешно разослано всем пользователям!</b>")

    # Очищаем состояние после рассылки
    await state.clear()
...
async def cancel_announcement(callback_query: CallbackQuery, state: FSMContext):
    await state.clear()
    await callback_query.message.edit_text("<b>❌ Рассылка отменена.</b>", reply_markup=get_admin_menu())
