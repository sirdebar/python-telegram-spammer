...
async def start_bot(message: Message, state: FSMContext):
    ...
    # Если пользователь есть в базе, проверим его подписку
    if result:
        is_subscribed, subscription_expires = result

        if is_subscribed and datetime.strptime(subscription_expires, "%Y-%m-%d %H:%M:%S") > datetime.now():
            # Проверка, подписан ли на канал через API
            ...
            if member_status.status in ["member", "administrator", "creator"]:
                ...
                # Обновляем статус подписки на 'не подписан'
                await update_subscription_status(user_id, False)
                sent_message = await message.answer("<b>Ваша подписка истекла или вы отписались от канала. Подпишитесь или введите ключ доступа.</b>", reply_markup=get_new_user_menu())
        else:
            # Если подписка истекла
            sent_message = await message.answer("<b>Ваша подписка истекла. Подпишитесь на канал или введите ключ доступа.</b>", reply_markup=get_new_user_menu())
    else:
...
async def check_subscription(callback_query: CallbackQuery, state: FSMContext):
    ...
    member_status = await bot.get_chat_member(chat_id=CHANNEL_ID, user_id=user_id)
    if member_status.status in ["member", "administrator", "creator"]:
        # Пользователь подписан, обновляем статус подписки в базе данных
        async with aiosqlite.connect('bot_database.db') as db:
            await db.execute('UPDATE users SET is_subscribed = ?, subscription_expires = ? WHERE user_id = ?', 
                             (True, (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d %H:%M:%S"), user_id))
            await db.commit()

        sent_message = await callback_query.message.answer("<b>🎉 Спасибо за подписку! Теперь купите доступ или введите ключ.</b>", reply_markup=get_new_user_menu())
    else:
