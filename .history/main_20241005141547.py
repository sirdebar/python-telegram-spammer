...
async def accept_agreement(callback_query: CallbackQuery, state: FSMContext):
    ...
    if "message_id" in previous_message_data:
        try:
            ...
            logging.error(f"Ошибка при удалении сообщения о соглашении: {e}")
    ...
    await check_subscription(callback_query.message, state)
...
async def check_subscription(callback_query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if callback_query.message:
            await callback_query.message.answer("<b>⚠️ Ошибка при проверке подписки. Пожалуйста, попробуйте позже.</b>")
        return
