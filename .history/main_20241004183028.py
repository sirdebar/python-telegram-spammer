...
async def show_profile(message: Message):
    ...
    if user_id == ADMIN_ID:
        ...
        cursor.execute('SELECT COUNT(*) FROM mailings WHERE user_id = ?', (ADMIN_ID,))
        personal_mailings = cursor.fetchone()[0]

        # Отправляем информацию администратору
        await message.answer(
            "<b>👑 Профиль администратора:</b>\n"
            f"<b>Имя:</b> Администратор\n\n"  # Разделение строк для читабельности
            f"<b>Активных подписчиков:</b> {active_subscribers}\n\n"
            f"<b>Количество всех рассылок:</b> {total_mailings}\n\n"
            f"<b>Количество ваших рассылок:</b> {personal_mailings}"
        )
        return

    # Если не администратор, показываем обычный профиль пользователя
    ...
    cursor.execute('SELECT COUNT(*) FROM mailings WHERE user_id = ?', (user_id,))
    ...
    await message.answer(
        ...
        f"<b>Количество рассылок:</b> {mailing_count}"
    )
