...
async def check_subscription(callback_query: CallbackQuery, state: FSMContext):
    ...
    if member_status.status in ["member", "administrator", "creator"]:
        ...
        # Пользователь не подписан на канал, отправляем инструкцию для подписки
        sent_message = await callback_query.message.answer("<b>Для использования бота, подпишитесь на канал:</b>", reply_markup=get_subscription_keyboard())
