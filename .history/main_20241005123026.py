...
async def confirm_send_announcement(callback_query: CallbackQuery, state: FSMContext):
    ...
    if data['content_type'] == "text":
        ...
        for user in users:
            try:
                ...
                errors.append(user_id)  # Запоминаем пользователей с ошибками
                if "business connection not found" not in str(e):
                    break  # Принудительно завершаем цикл при других ошибках
    ...
    # Редактирование сообщения только если оно НЕ фото
    try:
        if data['content_type'] == "text":
            # Если сообщение текстовое, мы можем редактировать текст
            await callback_query.message.edit_text("<b>✅ Сообщение успешно разослано всем пользователям!</b>")
        elif data['content_type'] == "photo":
            # Если сообщение с фото, НЕ редактируем, просто отправляем новое сообщение
        ...
        if "business connection not found" in str(e):
            logging.error("Сообщение не может быть отредактировано: business connection not found.")
        else:
            logging.error(f"Ошибка при редактировании сообщения: {e}")
        # Игнорируем эту ошибку и продолжаем выполнение
