...
async def start_bot(message: Message):
    ...
    if user_id == ADMIN_ID:
        await message.answer("Добро пожаловать, админ!", reply_markup=get_admin_menu())
        return
    ...
    if result:
        ...
        if datetime.strptime(subscription_expires, "%Y-%m-%d %H:%M:%S") > datetime.now():
            await message.answer("Добро пожаловать! Ваш доступ активен.", reply_markup=get_user_menu())
        else:
            await message.answer("Ваш доступ истек. Воспользуйтесь кнопками ниже.", reply_markup=get_new_user_menu())
    else:
        await message.answer("Добро пожаловать! Воспользуйтесь кнопками ниже.", reply_markup=get_new_user_menu())
...
async def show_profile(message: Message):
    ...
    await message.answer(f"Ваш профиль:\nИмя: {name}\nОставшийся срок подписки: {subscription_expires}\nКоличество рассылок: {mailing_count}")
...
async def process_key(message: Message, state: FSMContext):
    ...
    if result:
        ...
        if datetime.strptime(valid_until, "%Y-%m-%d %H:%M:%S") > datetime.now():
            ...
            await message.answer("Ключ активирован! Ваш доступ продлен.", reply_markup=get_user_menu())
        else:
...
async def add_account(message: Message, state: FSMContext):
    await message.answer("Введите номер телефона (в формате 9123456789).")
...
async def process_code(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Обрабатываем ошибку просроченного кода и запрашиваем новый код
        if "The confirmation code has expired" in str(e):
            await message.answer("Код истек. Отправляем новый...")
            await client.send_code_request(phone_number)
            await message.answer(f"Новый код отправлен на номер {phone_number}. Введите его.")
        else:
...
async def ask_for_days(message: Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        ...
        return
    await message.answer("Укажите срок действия ключа в днях:")
...
async def generate_key(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(f"Сгенерирован новый ключ на {days} дней: {key}")
        await state.clear()
...
async def choose_account(callback_query: CallbackQuery, state: FSMContext):
    ...
    if not chats:
        await callback_query.message.answer("Нет доступных чатов для этого аккаунта.")
        await state.clear()
        return
...
async def confirm_chats(callback_query: CallbackQuery, state: FSMContext):
    await callback_query.message.answer("Введите сообщения для рассылки (каждое сообщение с новой строки).")
...
async def process_delay(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("Рассылка запущена!", reply_markup=get_user_menu())
        await state.clear()

    except ValueError:
        await message.answer("Введите правильное число для задержки.")
