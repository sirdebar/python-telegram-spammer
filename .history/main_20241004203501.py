...
def get_user_menu():
    ...
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text='⚙️ Управление аккаунтами')]  # Новая кнопка для управления аккаунтами
        ],
        resize_keyboard=True
    )
    return keyboard
...
async def manage_accounts(message: Message):
    ...
    # Получаем список аккаунтов пользователя из базы данных
    cursor.execute('SELECT account_id, phone_number FROM accounts WHERE user_id = ?', (user_id,))
...
async def confirm_account_deletion(callback_query: CallbackQuery):
    ...
    # Получаем номер телефона по account_id
    cursor.execute('SELECT phone_number FROM accounts WHERE account_id = ?', (account_id,))
...
async def cancel_deletion(callback_query: CallbackQuery):
    """Отменяем удаление и возвращаем в главное меню"""
    # Здесь убираем `reply_markup=get_user_menu()` и передаем None
    await callback_query.message.edit_text("<b>Удаление аккаунта отменено.</b>")

    # Затем отправляем новое сообщение с основным меню (ReplyKeyboardMarkup)
...
async def delete_account(callback_query: CallbackQuery):
    ...
    # Получаем номер телефона и путь к сессии
    cursor.execute('SELECT phone_number FROM accounts WHERE account_id = ?', (account_id,))
    account = cursor.fetchone()
    phone_number = account[0]
    session_path = f'sessions/{phone_number}.session'

    # Подключаемся к сессии клиента
    client = TelegramClient(session_path, TELEGRAM_API_ID, TELEGRAM_API_HASH)
    await client.connect()

    # Убедимся, что клиент завершен перед удалением
    if client.is_connected():
        await client.disconnect()

    # Попытка удаления записи из базы данных с использованием блокировки
    with db_lock:
        retry_count = 0
        while retry_count < 3:  # Повторить попытку 3 раза
            try:
                ...
                if "database is locked" in str(e):
                    logging.warning(f"Database locked, retrying... Attempt {retry_count + 1}")
                    retry_count += 1
                    time.sleep(1)  # Задержка перед повторной попыткой
                else:
                    logging.error(f"Ошибка базы данных: {e}")
                    await callback_query.message.answer(f"<b>Ошибка базы данных: {e}</b>")
                    return
        else:
            await callback_query.message.answer("<b>Ошибка: не удалось удалить аккаунт из-за блокировки базы данных.</b>")
            return
    ...
    await callback_query.message.edit_text(
        ...
        reply_markup=None  # Удаляем клавиатуру, так как больше не нужно выводить кнопки
    )

    # Отправляем главное меню снова с ReplyKeyboardMarkup
    await callback_query.message.answer("<b>Вы вернулись в главное меню.</b>", reply_markup=get_user_menu())
...
async def show_profile(message: Message):
    ...
    if user_id == ADMIN_ID:
        # Подсчитываем количество активных подписчиков с неистекшей подпиской
        cursor.execute('SELECT COUNT(*) FROM users WHERE subscription_expires > ?', (datetime.now().strftime("%Y-%m-%d %H:%M:%S"),))
        active_subscribers = cursor.fetchone()[0] or 0  # Если нет подписчиков, возвращаем 0

        # Подсчитываем общее количество всех рассылок от всех пользователей
        cursor.execute('SELECT SUM(sent_messages) FROM mailings')
        total_mailings = cursor.fetchone()[0] or 0  # Если нет рассылок, возвращаем 0
        ...
        personal_mailings = cursor.fetchone()[0] or 0  # Если администратор не делал рассылки, возвращаем 0
    ...
    mailing_count = cursor.fetchone()[0] or 0  # Если нет рассылок, возвращаем 0
...
async def process_phone(message: Message, state: FSMContext):
    phone_number = message.text.strip()  # Убираем лишние пробелы и символы
    
    # Проверяем, является ли ввод корректным номером телефона
    if not phone_number.isdigit() or len(phone_number) < 10:  # Пример проверки для номеров
    ...
    try:
        ...
        await state.clear()  # Очищаем состояние FSM, если ошибка с номером
        return
    
    except Exception as e:
        await message.answer(f"<b>❌ Ошибка при обработке номера телефона: {e}</b>")
        await state.clear()  # Очищаем состояние FSM при любых других ошибках
        return
...
async def send_messages(account_id, chats, messages, delay, user_id):
    ...
    for chat in chats:
        try:
            await client.send_message(chat, messages)  # Отправляем сообщение в чат
            sent_count += 1
            cursor.execute(
            ...
            await asyncio.sleep(delay)  # Задержка между сообщениями, только один раз
...
async def start_mailing(account_id, chats, messages, photo, delay, user_id):
    ...
    # Главное изменение здесь — убираем задержку после цикла, чтобы она не удваивалась.
    while active_mailings.get(user_id):
        for chat_id in chats:
            ...
            try:
                ...
                # Ждем задержку только внутри цикла
...
# ================== ЗАВЕРШЕНИЕ РАССЫЛКИ И ПОДСЧЁТ ЛИЧНОЙ СТАТИСТИКИ ===================
...
async def stop_mailing(callback_query: CallbackQuery, state: FSMContext):
    ...
    if active_mailings.get(user_id):
        ...
        # Подсчёт сообщений и вычитание 1 для корректировки
        sent_messages_count = len(selected_chats) - 1  # Вычитаем 1 для исправления бага с удвоением
