...
class DeletePreviousMessageMiddleware(BaseMiddleware):
    async def __call__(self, handler: Callable[[Message, Dict[str, Any]], Awaitable[Any]], event: Message, data: Dict[str, Any]) -> Any:
        ...
        # Проверяем данные состояния пользователя для предыдущего сообщения бота
        previous_message_id = await state.get_data()

        # Удаляем предыдущее сообщение бота
        if "bot_message_id" in previous_message_id:
            try:
                await event.bot.delete_message(event.chat.id, previous_message_id["bot_message_id"])
            except Exception as e:
                logging.error(f"Ошибка удаления сообщения бота: {e}")

        # Вызов хендлера для обработки события
        result = await handler(event, data)

        # Сохраняем ID нового сообщения, отправленного ботом
        if result and isinstance(result, Message):
            await state.update_data(bot_message_id=result.message_id)
