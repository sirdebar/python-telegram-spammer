...
ADMIN_ID = 1234567890  # Ваш Telegram ID для доступа к админке
...
# ================== Инициализация Redis и RedisStorage ===================
...
# ================== Инициализация базы данных aiosqlite ===================
async def init_db():
...
async def confirm_account_deletion(callback_query: CallbackQuery):
    ...
    async with aiosqlite.connect('bot_database.db') as db:
        async with db.execute('SELECT phone_number FROM accounts WHERE account_id = ?', (account_id,)) as cursor:
            phone_number = (await cursor.fetchone())[0]
...
async def delete_account(callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_text(f"<b>Аккаунт с номером {phone_number} был успешно удалён.</b>")
...
async def show_profile(message: Message):
    ...
    async with aiosqlite.connect('bot_database.db') as db:
        if user_id == ADMIN_ID:
            async with db.execute('SELECT COUNT(*) FROM users WHERE subscription_expires > ?', (datetime.now().strftime("%Y-%m-%d %H:%M:%S"),)) as cursor:
                active_subscribers = (await cursor.fetchone())[0] or 0

            async with db.execute('SELECT SUM(sent_messages) FROM mailings') as cursor:
                total_mailings = (await cursor.fetchone())[0] or 0

            async with db.execute('SELECT SUM(sent_messages) FROM mailings WHERE user_id = ?', (ADMIN_ID,)) as cursor:
                personal_mailings = (await cursor.fetchone())[0] or 0
            ...
            async with db.execute('SELECT SUM(sent_messages) FROM mailings WHERE user_id = ?', (user_id,)) as cursor:
                mailing_count = (await cursor.fetchone())[0] or 0
...
async def process_code(message: Message, state: FSMContext):
    ...
    try:
        ...
        async with aiosqlite.connect('bot_database.db') as db:
            await db.execute('INSERT INTO accounts (user_id, phone_number, session) VALUES (?, ?, ?)', (message.from_user.id, phone_number, session_string))
            await db.commit()
    ...
    async with aiosqlite.connect('bot_database.db') as db:
        for dialog in dialogs:
            if isinstance(dialog.entity, InputPeerChannel):
                chat_id = dialog.id
                title = dialog.title
                await db.execute('INSERT OR REPLACE INTO chats (chat_id, account_id, title) VALUES (?, ?, ?)', (chat_id, account_id, title))
        await db.commit()
...
# ================== Рассылка сообщений (использование батчей и параллелизма) ===================
async def send_messages_batch(client, chat_id, messages, photo, delay):
    try:
        if photo:
...
async def start_mailing(account_id, chats, messages, photo, delay, user_id):
    ...
    while active_mailings.get(user_id):
        tasks = [send_messages_batch(client, chat_id, messages, photo, delay) for chat_id in chats]
