...
async def process_delay(message: Message, state: FSMContext):
    try:
        # Парсим задержку (ожидаем что пользователь укажет в формате 5с, 5м, 5ч)
        ...
        selected_chats = user_data['selected_chats']
        messages = user_data.get('messages')  # Может быть текстовое сообщение
        photo = user_data.get('photo')  # Может быть фотография
        caption = user_data.get('caption')  # Подпись к фото
        ...
        await state.clear()  # Сбрасываем состояние после старта рассылки
