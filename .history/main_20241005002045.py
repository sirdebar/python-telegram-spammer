...
@dp.message(state="waiting_for_key")
async def process_key(message: Message, state: FSMContext):
    key = message.text
    # Проверка ключа (например, простая проверка на валидный ключ, можно расширить)
    if key == "VALID_KEY":  # Замени на настоящую проверку ключа
        subscription_expires = (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d %H:%M:%S")  # Продление подписки на 30 дней
        async with aiosqlite.connect('bot_database.db') as db:
            await db.execute('UPDATE users SET subscription_expires = ? WHERE user_id = ?', (subscription_expires, message.from_user.id))
            await db.commit()

        await message.answer("<b>✅ Доступ активирован!</b> Добро пожаловать.", reply_markup=get_user_menu())
        await state.clear()
    else:
        await message.answer("<b>❌ Неверный ключ доступа.</b>")
        await state.clear()
    ...
    if result:
        ...
        if is_subscribed:
            if subscription_expires and datetime.strptime(subscription_expires, "%Y-%m-%d %H:%M:%S") > datetime.now():
                ...
                await message.answer("<b>Ваша подписка истекла. Купите доступ или введите ключ.</b>", reply_markup=get_new_user_menu())
        else:
            # Если не подписан на канал
            await message.answer("<b>Перед использованием бота, подпишитесь на канал:</b>", reply_markup=get_subscription_keyboard())
    else:
...
async def check_subscription(callback_query: CallbackQuery):
    ...
    if member_status.status in ["member", "administrator", "creator"]:
        ...
        async with aiosqlite.connect('bot_database.db') as db:
            subscription_expires = (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d %H:%M:%S")  # Пример: 30 дней подписки
            await db.execute('INSERT OR REPLACE INTO users (user_id, name, subscription_expires, is_subscribed) VALUES (?, ?, ?, ?)',
                             (user_id, callback_query.from_user.full_name, subscription_expires, True))
            await db.commit()

        await callback_query.message.answer("<b>🎉 Спасибо за подписку! Теперь купите доступ, чтобы использовать бота.</b>", reply_markup=get_new_user_menu())
    else:
