...
async def start_bot(message: Message, state: FSMContext):
    ...
    if result is None or not result[0]:  # Если соглашение не принято или пользователь новый
        ...
        sent_message = await message.answer(
            ...
            reply_markup=get_agreement_keyboard()  # Отправляем клавиатуру с соглашением
        )
        await state.update_data(message_id=sent_message.message_id)
        return  # Останавливаем выполнение функции, чтобы не проверять подписку
...
async def accept_agreement(callback_query: CallbackQuery, state: FSMContext):
    ...
    if "message_id" in previous_message_data:
        try:
            if callback_query.message:  # Проверяем наличие сообщения в callback_query
...
async def check_subscription(callback_query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback_query.answer("<b>⚠️ Ошибка при проверке подписки. Пожалуйста, попробуйте позже.</b>")
        return
    ...
    if member_status.status in ["member", "administrator", "creator"]:
        ...
        await callback_query.message.answer("<b>Для использования бота, подпишитесь на канал:</b>", reply_markup=get_subscription_keyboard())
        return
