...
async def start_mailing(account_id, chats, messages, photo, delay, user_id):
    ...
    while active_mailings.get(user_id):  # Рассылка активна
    ...
    # Отключаемся только если рассылка завершена (не при паузе)
    if not active_mailings.get(user_id):
        await client.disconnect()

        # Удаляем временные фото только после завершения рассылки, а не при паузе
...
async def stop_mailing(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Удаляем фото только при полном завершении рассылки
    if photo and os.path.exists(photo):
        os.remove(photo)
