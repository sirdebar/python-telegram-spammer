...
async def start_mailing(account_id, chats, messages, photo, delay, user_id):
    ...
    # Удаляем временные фото только после полного завершения рассылки (не при паузе)
    if photo and os.path.exists(photo) and not active_mailings.get(user_id):
        os.remove(photo)  # Удаляем файл, если он существует
