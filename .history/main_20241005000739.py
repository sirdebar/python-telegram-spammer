...
def get_subscription_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Подписаться", url="https://t.me/your_channel_link")],  # Ссылка на канал
...
async def start_bot(message: Message):
    ...
    if not result:
        ...
        if member_status.status in ["member", "administrator", "creator"]:
            async with aiosqlite.connect('bot_database.db') as db:
                subscription_expires = (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d %H:%M:%S")  # Срок подписки
                await db.execute('INSERT INTO users (user_id, name, subscription_expires) VALUES (?, ?, ?)',
                                (user_id, message.from_user.full_name, subscription_expires))
                await db.commit()
...
async def check_subscription(callback_query: CallbackQuery):
    ...
    if member_status.status in ["member", "administrator", "creator"]:
        async with aiosqlite.connect('bot_database.db') as db:
            ...
            await db.execute('INSERT INTO users (user_id, name, subscription_expires) VALUES (?, ?, ?)',
                            (user_id, callback_query.from_user.full_name, subscription_expires))
            await db.commit()
