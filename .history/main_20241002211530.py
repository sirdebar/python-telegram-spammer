...
async def start_bot(message: Message):
    if message.from_user.id == ADMIN_ID:
        # Отправляем меню для админа
        await message.answer("Привет, админ!", reply_markup=get_admin_menu())
    else:
        # Отправляем меню для обычного пользователя
        await message.answer("Добро пожаловать! Введите ключ доступа для активации или добавьте аккаунт.", reply_markup=get_user_menu())

# Хэндлер для входа в админку
...
async def show_profile(message: Message):
    cursor.execute('SELECT phone_number FROM accounts WHERE user_id = ?', (message.from_user.id,))
...
async def generate_key(callback_query: CallbackQuery):
    key = generate_random_key()
    await callback_query.message.answer(f"Сгенерирован новый ключ: {key}")
