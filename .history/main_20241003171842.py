...
async def choose_account(callback_query: CallbackQuery, state: FSMContext):
    ...
    if not chats:
        ...
        await get_chats(client, account_id)  # Парсим чаты
    ...
    # Генерируем клавиатуру для выбора чатов
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"{chat[1]} ✅", callback_data=f"toggle_chat_{chat[0]}")]
        for chat in chats
    ])

    await callback_query.message.answer("<b>Выберите чаты для рассылки:</b>", reply_markup=keyboard)
...
async def toggle_chat(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(f"Чат {'добавлен' if chat_id in selected_chats else 'удален'} для рассылки.")
