...
@dp.message(MailingStates.waiting_for_messages, F.text)
async def process_messages(message: Message, state: FSMContext):
    # Сохраняем сообщение с текстом в состояние
...
@dp.message(MailingStates.waiting_for_messages, F.photo)
async def process_photo_message(message: Message, state: FSMContext):
    # Получаем файл фото
    photo = message.photo[-1]  # Выбираем фото с максимальным разрешением
    caption = message.caption or ''  # Подпись к фото, если она есть
...
async def start_mailing(account_id, chats, messages, delay, user_id):
    """Запуск рассылки сообщений с поддержкой фото и текстовых сообщений."""
    ...
    sent_count = 0
    user_data = await dp.storage.get_data(user_id=user_id)
    photo = user_data.get('photo')  # Проверяем, есть ли фото
    caption = user_data.get('caption', '')  # Подпись к фото, если есть
    messages = user_data.get('messages', '')  # Текстовое сообщение, если фото нет
        ...
        for chat_id in chats:
            ...
            try:
                if photo:
                    ...
                    await client.send_file(chat_id, photo, caption=caption, parse_mode='HTML')
                else:
                    # Отправляем текстовое сообщение
                    await client.send_message(chat_id, messages, parse_mode='HTML')
                sent_count += 1
