    ...
    for chat in chats:
        try:
            if messages['type'] == "text":
                # Отправляем текстовое сообщение с эмодзи (премиум эмодзи тоже поддерживаются)
                await client.send_message(chat, messages['content'])
            elif messages['type'] == "photo":
                # Отправляем фото с подписью, поддерживая премиум эмодзи
