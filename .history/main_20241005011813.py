...
async def start_bot(message: Message, state: FSMContext):
    ...
    if result:
        ...
        if is_subscribed and subscription_expires and datetime.strptime(subscription_expires, "%Y-%m-%d %H:%M:%S") > datetime.now():
            # Если подписка активна, показываем "Ваш доступ активен"
    ...
    # Если пользователь не подписан или нет данных, проверяем подписку на канал
    else:
        member_status = await bot.get_chat_member(chat_id=CHANNEL_ID, user_id=user_id)
        if member_status.status in ["member", "administrator", "creator"]:
            # Пользователь подписан на канал, предлагаем покупку доступа
            ...
            async with aiosqlite.connect('bot_database.db') as db:
                await db.execute('INSERT OR REPLACE INTO users (user_id, name, is_subscribed) VALUES (?, ?, ?)', (user_id, message.from_user.full_name, True))
                await db.commit()
        else:
            # Пользователь не подписан на канал, предлагаем подписаться
