...
def get_terms_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Я согласен", callback_data="accept_terms")]
    ])
...
async def start_bot(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect('bot_database.db') as db:
        async with db.execute('SELECT is_subscribed, subscription_expires, accepted_terms FROM users WHERE user_id = ?', (user_id,)) as cursor:
            result = await cursor.fetchone()

    if result:
        is_subscribed, subscription_expires_str, accepted_terms = result
        ...
        if accepted_terms:
            ...
            if is_subscribed and subscription_expires > datetime.now():
                ...
                sent_message = await message.answer("<b>🎉 Спасибо за подписку! Теперь купите доступ для использования бота.</b>", reply_markup=get_new_user_menu())
        else:
            # Пользователь не принял соглашение, предложим принять
            sent_message = await message.answer("Примите пользовательское соглашение:", reply_markup=get_terms_keyboard())
            await state.update_data(message_id=sent_message.message_id)
            return  # Важно завершить обработку, чтобы не проходить дальше без принятия соглашения

    else:
        # Новый пользователь: предлагаем принять соглашение
        sent_message = await message.answer("Примите пользовательское соглашение:", reply_markup=get_terms_keyboard())
...
async def accept_terms(callback_query: CallbackQuery):
    user_id = callback_query.from_user.id
    # Обновляем запись в базе данных, что пользователь принял соглашение
    async with aiosqlite.connect('bot_database.db') as db:
        await db.execute('UPDATE users SET accepted_terms = 1 WHERE user_id = ?', (user_id,))
        await db.commit()

    # Переход к подписке после принятия соглашения
    await callback_query.message.answer("Подпишитесь на канал для продолжения:", reply_markup=get_subscription_keyboard())
