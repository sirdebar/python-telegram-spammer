    ...
    for chat in chats:
        try:
            if message_type == "text":
                # Отправляем текстовое сообщение, обрабатывая специальные символы и эмодзи
                text_message = messages_data["text"].encode('utf-16', 'surrogatepass').decode('utf-16')
                await client.send_message(chat, text_message)
            elif message_type == "photo":
                # Отправляем фото с подписью
                caption_message = messages_data["caption"].encode('utf-16', 'surrogatepass').decode('utf-16')
