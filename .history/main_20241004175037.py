...
async def process_messages(message: Message, state: FSMContext):
    # Если сообщение содержит фото, загружаем его
    if message.photo:
        ...
        # Сохраняем путь к фото в состояние
        await state.update_data(photo=photo_path)
        await state.update_data(messages=None)  # Обнуляем текст, если есть фото

    elif message.text:
        # Сохраняем текст сообщения
        await state.update_data(messages=message.text)
        await state.update_data(photo=None)  # Обнуляем фото, если есть текст
...
async def start_mailing(account_id, chats, messages, delay, user_id):
    ...
    user_data = await state.get_data()
    photo_path = user_data.get('photo')  # Получаем путь к фото, если оно было
    ...
    while active_mailings.get(user_id):
        for chat_id in chats:
            if not active_mailings.get(user_id):
                break
            try:
                # Отправляем фото или текст в зависимости от того, что есть
                if photo_path:
                    await client.send_file(chat_id, photo_path, caption=messages)
                else:
