    ...
    async def __call__(self, handler, event, data):
        ...
        if isinstance(event, Message):
            ...
            if user_id in self.previous_messages:
                try:
                    await event.bot.delete_message(chat_id, self.previous_messages[user_id])
                except Exception as e:
                    logging.error(f"Ошибка при удалении сообщения: {e}")

            # Обрабатываем сообщение и сохраняем ID отправленного сообщения
            result = await handler(event, data)
            self.previous_messages[user_id] = event.message_id

            return result
        else:
            # Если событие не является сообщением, просто продолжаем обработку
            return await handler(event, data)

# ================== РЕГИСТРАЦИЯ MIDDLEWARE ===================
dp.update.middleware(DeletePreviousMessageMiddleware())
