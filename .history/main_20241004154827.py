...
@dp.message(MailingStates.waiting_for_messages, F.content_type == 'text')
async def process_text_message(message: Message, state: FSMContext):
    # Обрабатываем текстовое сообщение
    await state.update_data(messages={"type": "text", "content": message.text})
...
@dp.message(MailingStates.waiting_for_messages, F.content_type == 'photo')
async def process_photo_message(message: Message, state: FSMContext):
    # Обрабатываем фото и подпись (если она есть)
    file_id = message.photo[-1].file_id  # Берем изображение максимального размера
    caption = message.caption or ""  # Подпись может быть пустой
    await state.update_data(messages={"type": "photo", "file_id": file_id, "caption": caption})
    await message.answer("<b>⏳ Введите задержку между сообщениями (например, 5с, 5м, 5ч, максимум 5 часов, минимум 5 секунд).</b>")
    await state.set_state(MailingStates.waiting_for_delay)
    ...
    for chat in chats:
        try:
            if messages['type'] == "text":
                # Отправляем текстовое сообщение, которое может содержать эмодзи
                await client.send_message(chat, messages['content'])  # Эмодзи автоматически поддерживаются
            elif messages['type'] == "photo":
                # Отправляем фотографию с подписью (если подпись присутствует)
                await client.send_file(chat, messages['file_id'], caption=messages['caption'])
            sent_count += 1
            await asyncio.sleep(delay)  # Задержка перед следующим сообщением
        except Exception as e:
