...
async def start_bot(message: Message, state: FSMContext):
    ...
    if "message_id" in previous_message_id:
        try:
            ...
            # Логируем ошибку, если сообщение не найдено или уже удалено
    ...
    # Проверяем статус подписки пользователя в базе данных
    async with aiosqlite.connect('bot_database.db') as db:
        async with db.execute('SELECT is_subscribed, subscription_expires FROM users WHERE user_id = ?', (user_id,)) as cursor:
            result = await cursor.fetchone()

    # Логика проверки подписки пользователя и выдача нужной клавиатуры
    if result:
        is_subscribed, subscription_expires = result

        # Проверяем подписку через API Telegram и обновляем базу данных
        member_status = await bot.get_chat_member(chat_id=CHANNEL_ID, user_id=user_id)
        if member_status.status in ["member", "administrator", "creator"]:
            if not is_subscribed:
                await update_subscription_status(user_id, True)  # Обновляем статус подписки
            ...
            await update_subscription_status(user_id, False)  # Обновляем статус в базе данных
...
async def check_subscription(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Проверяем статус подписки через API Telegram
    member_status = await bot.get_chat_member(chat_id=CHANNEL_ID, user_id=user_id)
    if member_status.status in ["member", "administrator", "creator"]:
        # Пользователь подписан, обновляем базу данных
        await update_subscription_status(user_id, True)

        # Возвращаемся к стандартному поведению /start
