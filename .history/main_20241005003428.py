...
async def choose_account(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Здесь будет код для показа чатов пользователя...
    
    # Отправка нового сообщения
    sent_message = await callback_query.message.answer("<b>Выберите чаты для рассылки:</b>", reply_markup=keyboard)
...
async def process_messages(message: Message, state: FSMContext):
    ...
    # Код для обработки сообщений и фото...
