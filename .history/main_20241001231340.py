...
def get_admin_menu():
    ...
    keyboard.add(KeyboardButton('👨‍💼 Войти в админку'))
    return keyboard
...
async def start_bot(message: Message):
    if message.from_user.id == ADMIN_ID:
        await message.answer("Привет, админ!", reply_markup=get_admin_menu())
    else:
        await message.answer("Добро пожаловать! Введите ключ доступа для активации.")
...
async def process_code(message: Message, state: FSMContext):
    ...
    if result:
        ...
        await message.answer(f"Ключ принят. Доступ активирован до: {valid_until}", reply_markup=get_user_menu())
        await state.clear()
    else:
...
async def new_mailing(message: Message, state: FSMContext):
    await message.answer("Пожалуйста, укажите список публичных каналов (ссылками через запятую).")
...
async def process_delay(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(f"Рассылка #**{mailing_id}** создана и запущена!", reply_markup=get_mailing_control_keyboard(mailing_id))
...
async def send_messages(mailing_id, chats, messages, delay):
    ...
    for chat in chats:
        # Присоединяемся к каналу (если это публичный канал, то это возможно)
        await bot.join_chat(chat.strip())
...
async def cancel_mailing(callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_text(f"Рассылка #**{mailing_id}** прервана.")
