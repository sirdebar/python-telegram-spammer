...
from aiogram.types import Message, InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, ReplyKeyboardMarkup, KeyboardButton, Message, ContentType
...
class DeletePreviousBotMessageMiddleware(BaseMiddleware):
    def __init__(self):
        super().__init__()
        # Словарь для хранения ID последнего сообщения от бота (ключ — ID пользователя)
        self.previous_bot_messages = {}

    async def __call__(self, handler, event, data):
        if isinstance(event, Message):
            bot: Bot = data['bot']  # Доступ к объекту бота
            user_id = event.from_user.id
            chat_id = event.chat.id

            # Если есть предыдущее сообщение от бота, удаляем его
            if user_id in self.previous_bot_messages:
                previous_message_id = self.previous_bot_messages[user_id]
                try:
            ...
            # Теперь вызываем хендлер для отправки нового сообщения
            result = await handler(event, data)

            # Если результатом обработки было сообщение, сохраняем его ID
            if isinstance(result, Message):
                self.previous_bot_messages[user_id] = result.message_id

            return result
        else:
            # Если событие не является сообщением, продолжаем обработку
            return await handler(event, data)

# ================== РЕГИСТРАЦИЯ MIDDLEWARE ===================
dp.message.middleware(DeletePreviousBotMessageMiddleware())
...
def get_subscription_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Подписаться", url="https://t.me/diablocatos")],  # Ссылка на канал
...
async def start_bot(message: Message, state: FSMContext):
    ...
    # Проверяем, подписан ли пользователь на канал и истекла ли подписка
    async with aiosqlite.connect('bot_database.db') as db:
        async with db.execute('SELECT is_subscribed, subscription_expires FROM users WHERE user_id = ?', (user_id,)) as cursor:
            result = await cursor.fetchone()

    if result:
        is_subscribed, subscription_expires = result
        if is_subscribed:
            if subscription_expires and datetime.strptime(subscription_expires, "%Y-%m-%d %H:%M:%S") > datetime.now():
                # Если подписка на бота активна
                sent_message = await message.answer("<b>🎉 Добро пожаловать!</b> Ваш доступ <b>активен</b>.", reply_markup=get_user_menu())
            else:
                # Если подписка на бота истекла
                sent_message = await message.answer("<b>Ваша подписка истекла. Купите доступ или введите ключ.</b>", reply_markup=get_new_user_menu())
        else:
            # Если не подписан на канал
            sent_message = await message.answer("<b>Перед использованием бота, подпишитесь на канал:</b>", reply_markup=get_subscription_keyboard())
    else:
        # Пользователь не зарегистрирован в базе данных, предложим подписку
        sent_message = await message.answer("<b>Перед использованием бота, подпишитесь на канал:</b>", reply_markup=get_subscription_keyboard())
...
async def check_subscription(callback_query: CallbackQuery, state: FSMContext):
    ...
    if member_status.status in ["member", "administrator", "creator"]:
        # Пользователь подписан, обновляем базу
        async with aiosqlite.connect('bot_database.db') as db:
            await db.execute('UPDATE users SET is_subscribed = ? WHERE user_id = ?', (True, user_id))
            await db.commit()

        sent_message = await callback_query.message.answer("<b>🎉 Спасибо за подписку! Теперь купите доступ, чтобы использовать бота.</b>", reply_markup=get_new_user_menu())
    else:
...
async def main():
    ...
    dp.message.middleware(DeletePreviousBotMessageMiddleware())  # Регистрируем middleware
