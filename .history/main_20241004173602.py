    ...
    for chat in chats:
        try:
            if message_type == "text":
                ...
                await client.send_message(chat, messages_data["text"])
            elif message_type == "photo":
                # Отправляем фото с подписью (если есть)
                await client.send_file(chat, messages_data["photo"], caption=messages_data["caption"])
            sent_count += 1
            with db_lock:
                cursor.execute('INSERT INTO mailings (user_id, chats, messages, status, sent_messages, start_time) VALUES (?, ?, ?, ?, ?, ?)',
                               (user_id, ','.join(str(c) for c in chats), '\n'.join(str(messages_data)), 'active', sent_count, datetime.now().isoformat()))
                conn.commit()
