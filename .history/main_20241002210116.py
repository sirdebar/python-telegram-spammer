...
ADMIN_ID = YOUR_TELEGRAM_USER_ID  # Ваш Telegram ID для доступа к админке
...
cursor.execute('''CREATE TABLE IF NOT EXISTS accounts (account_id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, phone_number TEXT, session TEXT)''')
...
cursor.execute('''CREATE TABLE IF NOT EXISTS chats (account_id INTEGER, chat_id INTEGER, title TEXT)''')
conn.commit()
...
async def choose_account(callback_query: CallbackQuery, state: FSMContext):
    ...
    chats = зх[{'id': row[0], 'title': row[1]}] for row in cursor.fetchall()]
