...
async def check_subscription(callback_query: CallbackQuery):
    ...
    if member_status.status in ["member", "administrator", "creator"]:
        ...
        async with aiosqlite.connect('bot_database.db') as db:
            await db.execute('INSERT OR REPLACE INTO users (user_id, name, is_subscribed) VALUES (?, ?, ?)',
                             (user_id, callback_query.from_user.full_name, True))
            await db.commit()

        await callback_query.message.answer("<b>🎉 Спасибо за подписку! Доступ активирован.</b>", reply_markup=get_user_menu())
    else:
