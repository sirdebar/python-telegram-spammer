...
async def change_channel_prompt(message: Message, state: FSMContext):
    ...
    if user_id == ADMIN_ID:
        await message.answer("<b>Введите айди нового канала для обязательной подписки:</b>")
        await state.set_state(ChannelChangeStates.waiting_for_channel_id)
    else:
        await message.answer("<b>⛔ У вас нет прав на изменение канала для обязательной подписки.</b>")

# Обработка ввода нового ID канала
@dp.message(ChannelChangeStates.waiting_for_channel_id)
async def set_new_channel_id(message: Message, state: FSMContext):
    global CHANNEL_ID

    try:
        new_channel_id = int(message.text)  # Преобразуем ввод в целое число
        CHANNEL_ID = new_channel_id  # Обновляем глобальную переменную
        await message.answer(f"<b>✅ Обязалка успешно изменена. Новый канал ID: {CHANNEL_ID}</b>", reply_markup=get_admin_menu())
        await state.clear()  # Очищаем состояние
    except ValueError:
        await message.answer("<b>⚠️ Ошибка: Введите корректный числовой ID канала.</b>")
