...
async def start_bot(message: Message, state: FSMContext):
    ...
    # Если пользователь не принял соглашение или отсутствует в БД
    if result is None or not result[0]:  
        logging.info(f"User {user_id} has not accepted terms. Sending agreement message.")
    ...
    # Если пользователь уже согласился, продолжаем с проверкой подписки
    await check_subscription(message, state)

@dp.callback_query(Text("accept_agreement"))  # Используйте Text вместо F.data
async def accept_agreement(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Переходим к следующему этапу — проверка подписки на канал
...
async def check_subscription(event: Union[Message, CallbackQuery], state: FSMContext):
    ...
    # Определяем объект сообщения (Message или CallbackQuery)
    ...
    if "message_id" in previous_message_data:
        try:
            await bot.delete_message(chat_id=message_obj.chat.id, message_id=previous_message_data['message_id'])
        except Exception as e:
            logging.error(f"Ошибка при удалении сообщения: {e}")
    ...
    try:
        ...
        await message_obj.answer("<b>⚠️ Ошибка при проверке подписки. Пожалуйста, попробуйте позже.</b>")
        return
    ...
    if member_status.status in ["member", "administrator", "creator"]:
        ...
        # Теперь проверяем платную подписку
        async with aiosqlite.connect('bot_database.db') as db:
        ...
        if result:
            ...
            if is_subscribed and subscription_expires and subscription_expires > datetime.now():
                sent_message = await message_obj.answer("<b>🎉 Спасибо за подписку! Ваш доступ активен.</b>", reply_markup=get_user_menu())
            else:
                sent_message = await message_obj.answer("<b>🎉 Спасибо за подписку! Теперь купите доступ для использования бота.</b>", reply_markup=get_new_user_menu())
        else:
            # Пользователь есть в БД, но нет данных о подписке
            sent_message = await message_obj.answer("<b>🎉 Спасибо за подписку! Теперь купите доступ для использования бота.</b>", reply_markup=get_new_user_menu())
    else:
        # Пользователь не подписан на канал, отправляем сообщение с просьбой подписаться
        sent_message = await message_obj.answer("<b>Для использования бота, подпишитесь на канал:</b>", reply_markup=get_subscription_keyboard())
        return
