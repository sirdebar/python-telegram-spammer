...
class DeletePreviousBotMessageMiddleware(BaseMiddleware):
    def __init__(self):
        super().__init__()
        # Словарь для хранения ID предыдущих сообщений, отправленных ботом (по пользователям)
    ...
    async def __call__(self, handler, event, data):
        if isinstance(event, Message):
            ...
            # Если существует предыдущее сообщение от бота, удаляем его
            ...
            # Обрабатываем текущее событие (отправляем новое сообщение)
            result = await handler(event, data)

            # Сохраняем ID последнего сообщения, отправленного ботом
            if isinstance(result, Message):
                self.previous_bot_messages[user_id] = result.message_id

            return result
        else:
            # Если событие не является сообщением, продолжаем обычную обработку
