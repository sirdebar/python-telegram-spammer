...
async def process_messages(message: Message, state: FSMContext):
    ...
    if message.photo:
        # Если в сообщении есть фото, сохраняем его
        ...
        if message.caption:
            ...
            await state.update_data(messages="")  # Пустое сообщение, если только фото
    else:
...
def parse_delay(delay_input):
    """
    Функция для парсинга задержки в формате:
    - '5с' (секунды)
    - '5м' (минуты)
    - '5ч' (часы)
    Возвращает задержку в секундах.
    """
    match = re.match(r"(\d+)([ссмч])", delay_input.lower())
    if match:
...
async def process_delay(message: Message, state: FSMContext):
    try:
        delay_input = message.text
        delay = parse_delay(delay_input)  # Парсим задержку
        
        if delay is None or delay < 5 or delay > 18000:  # Минимум 5 секунд, максимум 5 часов (18000 секунд)
            raise ValueError
...
async def start_mailing(account_id, chats, messages, delay, user_id):
    ...
    # Получаем данные из состояния (сообщение и фото, если есть)
    ...
    while active_mailings.get(user_id):
        for chat_id in chats:
            ...
            try:
                if photo:
                    ...
                    await client.send_file(chat_id, photo, caption=messages)
                else:
        ...
        if active_mailings.get(user_id):
            await asyncio.sleep(delay)  # Цикл повторяется с задержкой
...
async def stop_mailing(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Удаляем временные фото, если они были
    user_data = await state.get_data()
    photo_path = user_data.get('photo')
    if photo_path and os.path.exists(photo_path):
        os.remove(photo_path)  # Удаляем фото из временной директории
