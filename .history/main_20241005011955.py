...
async def process_key(message: Message, state: FSMContext):
    ...
    if result:
        ...
        if datetime.strptime(valid_until, "%Y-%m-%d %H:%M:%S") > datetime.now():
            ...
            async with aiosqlite.connect('bot_database.db') as db:
                await db.execute('INSERT OR REPLACE INTO users (user_id, name, subscription_expires, is_subscribed) VALUES (?, ?, ?, ?)',
                                (message.from_user.id, message.from_user.full_name, subscription_expires, True))
                await db.commit()

            await message.answer("<b>✅ Ключ активирован! Ваш доступ <b>активен</b>.</b>", reply_markup=get_user_menu())
        else:
...
async def start_bot(message: Message, state: FSMContext):
    ...
    # Если пользователь не найден в базе, проверяем его подписку на канал
    if not result:
        member_status = await bot.get_chat_member(chat_id=CHANNEL_ID, user_id=user_id)
        if member_status.status in ["member", "administrator", "creator"]:
            # Пользователь подписан на канал, но еще не зарегистрирован в базе
            sent_message = await message.answer("<b>🎉 Спасибо за подписку! Купите доступ, чтобы использовать бота.</b>", reply_markup=get_new_user_menu())
            # Сохраняем пользователя в базе как подписанного
            async with aiosqlite.connect('bot_database.db') as db:
                await db.execute('INSERT INTO users (user_id, name, is_subscribed) VALUES (?, ?, ?)', (user_id, message.from_user.full_name, True))
                await db.commit()
        else:
    ...
    # Если пользователь найден в базе
        ...
        # Проверяем, подписан ли пользователь и есть ли активный доступ
        if is_subscribed and subscription_expires and datetime.strptime(subscription_expires, "%Y-%m-%d %H:%M:%S") > datetime.now():
            # Если доступ активен, показываем сообщение "Ваш доступ активен"
            sent_message = await message.answer("<b>🎉 Добро пожаловать! Ваш доступ <b>активен</b>.</b>", reply_markup=get_user_menu())
        else:
            # Если срок подписки истек или доступ не был куплен
...
async def clear_all_chats(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Очищаем выбранные чаты
    await state.update_data(selected_chats=[])

    # Обновляем интерфейс выбора чатов
