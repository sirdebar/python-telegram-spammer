...
async def process_announcement_content(message: Message, state: FSMContext):
    ...
    # Сохраняем ID сообщения с предпросмотром, чтобы потом удалить его при необходимости
    await state.update_data(message_id=sent_message.message_id)

    # Переходим к этапу подтверждения рассылки
...
async def confirm_send_announcement(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Отправляем администратору сообщение об успешной рассылке
    try:
        if callback_query.message.content_type == "photo":
            await callback_query.message.edit_caption("<b>✅ Сообщение успешно разослано всем пользователям!</b>")
        else:
            await callback_query.message.edit_text("<b>✅ Сообщение успешно разослано всем пользователям!</b>")
    except Exception as e:
    ...
    await callback_query.message.answer("<b>Вы вернулись в главное меню</b>", reply_markup=get_admin_menu())
...
async def cancel_announcement(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Уведомляем об отмене рассылки
    try:
        await callback_query.message.edit_text("<b>❌ Рассылка отменена.</b>", reply_markup=InlineKeyboardMarkup())
    except Exception as e:
        logging.error(f"Ошибка при редактировании сообщения об отмене: {e}")

    # Возвращаем меню администратора
    await callback_query.message.answer("<b>Вы вернулись в главное меню</b>", reply_markup=get_admin_menu())
    await state.clear()

# Обработка текста или фото для объявления с удалением предыдущего сообщения
@dp.message(AnnouncementStates.waiting_for_content, F.text | F.photo)
async def process_announcement_content(message: Message, state: FSMContext):
    # Удаляем предыдущее сообщение, где запрашивалось ввести текст или фото
    previous_message_data = await state.get_data()
    if "intro_message_id" in previous_message_data:
        try:
            await bot.delete_message(chat_id=message.chat.id, message_id=previous_message_data['intro_message_id'])
        except Exception as e:
            logging.error(f"Ошибка при удалении предыдущего сообщения: {e}")
...
async def start_bot(message: Message, state: FSMContext):
    ...
    # Проверка подписки пользователя в базе данных
    async with aiosqlite.connect('bot_database.db') as db:
    ...
    if result:
        ...
        # Проверка, если подписка активна
...
async def check_subscription(callback_query: CallbackQuery, state: FSMContext):
    ...
    if member_status.status in ["member", "administrator", "creator"]:
        ...
        if result:
            ...
            # Если пользователя нет в базе, предлагаем оформить платную подписку
