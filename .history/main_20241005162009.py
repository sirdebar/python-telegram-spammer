...
async def check_subscription(callback_query: CallbackQuery, state: FSMContext):
    ...
    sent_message = None  # Инициализируем переменную по умолчанию
            ...
            if result:
                ...
                if is_subscribed and subscription_expires > datetime.now():
                    sent_message = await callback_query.message.answer(
                        "<b>🎉 Спасибо за подписку! Ваш доступ активен.</b>", 
                        reply_markup=get_user_menu()
                    )
                else:
                    sent_message = await callback_query.message.answer(
                        "<b>🎉 Спасибо за подписку! Теперь купите доступ для использования бота.</b>", 
    ...
    if sent_message:  # Проверяем, был ли отправлен ответ перед обновлением данных
        await state.update_data(message_id=sent_message.message_id)
