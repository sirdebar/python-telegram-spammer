...
async def check_subscription(callback_query: CallbackQuery):
    ...
    try:
        ...
        if member.status in ['member', 'administrator', 'creator']:
            # Удаляем сообщение о необходимости подписки
            ...
            raise TelegramBadRequest  # Если не подписан, повторяем сообщение
        ...
        for user in users:
            ...
            try:
                ...
                if member.status != 'member':
                    # Если пользователь отписался, обновляем базу данных и отправляем уведомление
                    await db.execute('UPDATE users SET subscribed = ? WHERE user_id = ?', (False, user_id))
