...
from aiogram.types import Message, InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, ReplyKeyboardMarkup, KeyboardButton, InputFile
...
async def process_phone(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"<b>❌ Номер телефона {phone_number} не зарегистрирован в Telegram.</b>")
        await state.clear()
...
async def process_photo_message(message: Message, state: FSMContext):
    photo = message.photo[-1]  # Берём наибольшее разрешение фото
    caption = message.caption if message.caption else ""  # Подпись, если есть
    await state.update_data(photo=photo.file_id, caption=caption)
    await message.answer("<b>📷 Изображение с подписью принято.</b>")
    # Переходим на этап ожидания задержки
    await message.answer("<b>⏳ Введите задержку между сообщениями (например, 5с, 5м, 5ч).</b>")
    await state.set_state(MailingStates.waiting_for_delay)  # Переход к ожиданию задержки
...
async def process_text_message(message: Message, state: FSMContext):
    ...
    await message.answer("<b>📝 Текстовое сообщение принято.</b>")
    # Переходим на этап ожидания задержки
    await message.answer("<b>⏳ Введите задержку между сообщениями (например, 5с, 5м, 5ч).</b>")
    await state.set_state(MailingStates.waiting_for_delay)  # Переход к ожиданию задержки
...
def parse_delay(delay_str):
    ...
    if delay_str.endswith('с'):
        return int(delay_str[:-1])  # Задержка в секундах
    elif delay_str.endswith('м'):
        return int(delay_str[:-1]) * 60  # Задержка в минутах
    elif delay_str.endswith('ч'):
        return int(delay_str[:-1]) * 3600  # Задержка в часах
    else:
        raise ValueError("Неверный формат задержки. Используйте 'с', 'м' или 'ч'.")
...
async def process_delay(message: Message, state: FSMContext):
    try:
        ...
        if delay < 5 or delay > 18000:  # Ограничение от 5 секунд до 5 часов
            raise ValueError
        ...
        # Запуск рассылки
        asyncio.create_task(start_mailing(account_id, selected_chats, messages, delay, message.from_user.id, photo, caption))
        await message.answer("<b>🚀 Рассылка запущена!</b> Статус: <b>Активна</b>")
        await state.clear()  # Очищаем состояние после старта рассылки

    except ValueError:
        await message.answer("<b>⚠️ Неверный формат. Введите задержку в формате 5с, 5м, 5ч (от 5 секунд до 5 часов).</b>")
...
async def send_messages(account_id, chats, messages, delay, user_id, photo=None, caption=None):
    cursor.execute('SELECT phone_number, session FROM accounts WHERE account_id = ?', (account_id,))
    account = cursor.fetchone()
    ...
    # Отправляем первое сообщение сразу во все чаты
    for chat in chats:
        try:
            if photo:
                # Если есть изображение
                await client.send_file(chat, InputFile(photo), caption=caption, parse_mode='html')
            else:
                # Если это текстовое сообщение
                await client.send_message(chat, messages, parse_mode='html')

            sent_count += 1

            with db_lock:  # Синхронизация доступа к базе данных
                cursor.execute('INSERT INTO mailings (user_id, chats, messages, status, sent_messages, start_time) VALUES (?, ?, ?, ?, ?, ?)',
                               (user_id, ','.join(str(c) for c in chats), caption if photo else messages, 'active', sent_count, datetime.now().isoformat()))
                conn.commit()

        except Exception as e:
            logging.error(f"Ошибка при отправке сообщения в чат {chat}: {e}")

    # Задержка перед следующим циклом отправки сообщений
    await asyncio.sleep(delay)

    # Теперь отправляем последующие сообщения с задержкой
    while True:
        for chat in chats:
            try:
                if photo:
                ...
                sent_count += 1
                with db_lock:
                    cursor.execute('UPDATE mailings SET sent_messages = ? WHERE user_id = ? AND chats = ?', (sent_count, user_id, ','.join(str(c) for c in chats)))
                    conn.commit()
                await asyncio.sleep(delay)  # Ждём задержку
            except Exception as e:
...
# Переопределение методов работы с базой данных для избежания блокировок
def execute_with_lock(query, params=()):
    with db_lock:
        cursor.execute(query, params)
        conn.commit()

def executemany_with_lock(query, param_list):
    with db_lock:
