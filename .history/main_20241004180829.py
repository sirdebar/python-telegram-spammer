...
async def start_mailing(account_id, chats, messages, photo, delay, user_id, client):
    """Запуск рассылки сообщений с контролем и циклической отправкой."""
    active_mailings[user_id] = True  # Рассылка активна
    sent_count = 0

    # Цикл для отправки сообщений с паузами
    while active_mailings.get(user_id):
        for chat_id in chats:
            if not active_mailings.get(user_id):  # Если рассылка приостановлена, выходим из цикла
                break
            try:
    ...
    # Завершаем рассылку и отключаем клиента, только если это полный стоп
    if not active_mailings.get(user_id):
        await client.disconnect()

        # Удаляем временные фото только после завершения рассылки
...
async def pause_mailing(callback_query: CallbackQuery, state: FSMContext):
    ...
    if active_mailings.get(user_id):
        active_mailings[user_id] = False  # Останавливаем рассылку (пауза), но не отключаем клиента
...
async def resume_mailing(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Подключение клиента из состояния
    client = await state.get_data().get("client")

    if not client or not client.is_connected():
        # Если клиент не подключен, повторно подключаемся
    ...
    asyncio.create_task(start_mailing(account_id, selected_chats, messages, photo, delay, callback_query.from_user.id, client))
...
async def stop_mailing(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Получаем путь к фото из состояния, если оно есть
    user_data = await state.get_data()
    photo = user_data.get('photo')

    # Отключаем клиента и удаляем фото
    client = await state.get_data().get("client")
