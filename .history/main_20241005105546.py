...
async def start_bot(message: Message, state: FSMContext):
    ...
    # Проверяем подписку пользователя в базе данных
    async with aiosqlite.connect('bot_database.db') as db:
        async with db.execute('SELECT is_subscribed, subscription_expires FROM users WHERE user_id = ?', (user_id,)) as cursor:
            result = await cursor.fetchone()

    # Если результат есть, проверим подписку
    if result:
        is_subscribed, subscription_expires = result
        
        # Если подписка активна, проверим через API Telegram, подписан ли пользователь на канал
        if is_subscribed:
            member_status = await bot.get_chat_member(chat_id=CHANNEL_ID, user_id=user_id)
            
            if member_status.status in ["member", "administrator", "creator"] and datetime.strptime(subscription_expires, "%Y-%m-%d %H:%M:%S") > datetime.now():
                # Если пользователь подписан и подписка еще действительна
                sent_message = await message.answer("<b>🎉 Добро пожаловать!</b> Ваш доступ <b>активен</b>.", reply_markup=get_user_menu())
            else:
                # Подписка истекла или пользователь отписался
                await update_subscription_status(user_id, False)  # Обновим статус подписки
                sent_message = await message.answer("<b>Ваша подписка истекла или вы отписались от канала. Купите доступ или введите ключ.</b>", reply_markup=get_new_user_menu())
        else:
...
async def show_chat_selection(message, chats, state: FSMContext, page=0):
    ...
    # Если первый раз загружаем чаты, все чаты должны быть включены (selected)
    if not selected_chats:
        selected_chats = [chat[0] for chat in chats]  # Все чаты включены по умолчанию

    buttons = []
    for chat in current_page_chats:
        status = "✅" if chat[0] in selected_chats else "❌"  # Ставим крестик ❌ для отключенных чатов
        buttons.append([InlineKeyboardButton(text=f"{chat[1]} {status}", callback_data=f"toggle_chat_{chat[0]}")])

    # Пагинация, если чатов больше, чем помещается на страницу
    ...
    # Кнопка "Убрать все" теперь очищает все выбранные чаты
...
async def clear_all_chats(callback_query: CallbackQuery, state: FSMContext):
    ...
    selected_chats = []  # Все чаты выключены (пустой список)
    await state.update_data(selected_chats=selected_chats)

    # Обновляем интерфейс с чаты в состоянии "❌" (выключены)
