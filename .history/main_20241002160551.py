...
async def start_bot(message: Message):
    if message.from_user.id == ADMIN_ID:
        escaped_text = escape_md("Привет, админ!")
        await message.answer(escaped_text, reply_markup=get_admin_menu(), parse_mode="MarkdownV2")
    else:
        await message.answer("Добро пожаловать! Введите ключ доступа для активации.", parse_mode="MarkdownV2")
@dp.message(F.text == "👨‍💼 Войти в админку")
async def admin_panel(message: Message):
    if message.from_user.id != ADMIN_ID:
        await message.answer("У вас нет доступа к этой команде.", parse_mode="MarkdownV2")
        return

    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Генерация ключа", callback_data="generate_key")]
    ])
    await message.answer("*Админ-панель*:", reply_markup=keyboard, parse_mode="MarkdownV2")
